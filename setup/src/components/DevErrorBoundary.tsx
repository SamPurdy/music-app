 ...
 * visibly on-screen in development so local AI models (and developers) can see
 * what broke without needing to open browser DevTools.
 *
 * USAGE:
 *   This component is automatically applied in src/main.tsx during development:
 *     import.meta.env.DEV ? <DevErrorBoundary><App /></DevErrorBoundary> : <App />
 *   You do not need to use it directly anywhere else.
 *
 * NOTE: React requires error boundaries to be class components (as of React 18).
 * This is the ONE intentional exception to the "no class components" rule.
 * Do not convert this to a function component — it will not work.
 *
 * WHAT IT CATCHES:
 ...
 *   ❌ Errors in async code (those show as the toast in index.html)
 *
